$('input#busc-submit').on('click',function(){
		...
		if ($.trim(busc) != ''){
			$.post("http://192.168.43.142/sgr/index.php/comanda/busc/",{busc: busc, comandamesa: comandamesa}, function(data){
				$('#busc-data').html(data);
			});
		}
});
$('input#entradas-submit').on('click',function(){
		...
		var idTipo = document.getElementById('idTipo1').value;
			$.post("http://192.168.43.142/sgr/producto/listarCat/",{idTipo: idTipo,comandamesa: comandamesa}, function(data){
				$('#busc-data').html(data);
			});
});
